        ...
        try {
            ...
            for (let i = 1; i <= pdf.numPages; i++) {
                ...
                const items: TextItem[] = textContent.items
                    .filter((item: any) => item.str.trim().length > 0) as TextItem[];

                // 1. Sort by Y (Top to Bottom) - PDF Y is bottom-left origin
                ...
                // 3. Detect Global Columns (Clustering X coordinates)
                ...
                const sheetData: any[][] = rows.map(row => {
                    ...
                    row.forEach(item => {
                        ...
                        if (minDist < 50) {
                            ...
                            if (!isNaN(num) && val !== "") {
                                rowData[bestColIndex] = num;
                            } else {
                                rowData[bestColIndex] = val;
                            }
                        }
                    });
                    return rowData;
            ...
            toast.error("Failed to convert PDF. Complex tables might not extract perfectly.");
        } finally {
        ...
        <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8 font-sans selection:bg-blue-500/30">
            <div className="max-w-6xl mx-auto space-y-8">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-800">
                    <div className="space-y-2">
                        ...
                        <p className="text-slate-400 max-w-lg leading-relaxed">
                            Analyze PDF layout to extract tables and data into Excel spreadsheets.
                        </p>
                    </div>
                                ...
                                >
                                    ...
                                    <div className="relative z-10 text-center space-y-8">
                                        <div className="relative w-24 h-24 mx-auto">
                                            ...
                                            <div className="relative w-full h-full rounded-2xl bg-slate-950 border border-emerald-500/30 flex items-center justify-center text-blue-400 shadow-2xl">
                                                <Database className="w-10 h-10" />
                                            </div>
                                            <div className="absolute -bottom-2 -right-2 bg-blue-500 rounded-full p-1.5 border-4 border-slate-900">
                                                <CheckCircle className="w-4 h-4 text-white" />
                                            </div>
                                        </div>
                                        <div className="space-y-3">
                                            <h3 className="text-2xl font-black uppercase tracking-tight text-white">Ready to Extract</h3>
                                ...
                                <div className="space-y-4 mb-8">
                                    <div className="p-4 rounded-xl bg-blue-500/5 border border-blue-500/10 mb-4">
                                        <p className="text-xs text-blue-300 leading-relaxed">
                                            <strong>Note:</strong> We analyze visual alignment to detect columns. Best for reports and simple tables.
                                        </p>
                                    </div>
                                </div>
                ...
                <DownloadConfirmDialog
                    ...
                    defaultFileName={selectedFile?.name.replace('.pdf', '') || "spreadsheet"}
                    extension="xlsx"
                    isProcessing={isProcessing}
                    title="Save Excel Sheet"
                    description="Enter a name for your converted spreadsheet."
                />
            </div>
        </div>
    );
